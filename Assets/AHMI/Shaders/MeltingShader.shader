    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (VertexInput v)
            {
                v2f o;
                o.base = vertBase(v);
                o.worldPos = mul(unity_ObjectToWorld, v.vertex).xyz; //get object's world position
            ...
            {
                half4 col = fragBase(i.base); //get base pixel color 
                float draw = 0;
                    ...
                    for (int j = 0; j < _CoordinatesCount; j++) {
                        ...
                        if(posdistance <  1+_Power[j]) {
                            float draw2 = pow(1+_Power[j]-posdistance,8);
                                ...
                                draw += draw2; //add melting color based on melting points
                        }
                    }
                }
                fixed4 drawcol = _PColor * (draw);
                return saturate(col+drawcol); //draw base color + melting color.
            }
